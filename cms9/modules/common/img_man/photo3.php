...
if(isset($upd)) 
{
	...
	if($_POST['removeTo'] != '' && $_POST['removeTo'] != 0)
	{
		...
		if($copy)
		{
			...
			$sql = "insert into `$tbl` set `name`='$name', `file_ext`='$ext', `tags`='$tags', url='$url', img_show = '$img_show'";
			mysql_query($sql);
			//echo $sql.'<br>';
	...
	{
		isset($_POST['img_show']) ? $c = '1' : $c = '0';
		$sql = "update `$tbl` set `name`='$name', `tags`='$tags', `url`='$url', img_show = '".$c."' where `id` = '$id'";
		mysql_query($sql);	
...
$sql = "ALTER TABLE `$tbl` ADD `img_show` ENUM('1','0') NOT NULL";
$res = mysql_query($sql);
/*$sql = "ALTER TABLE `$tbl` CHANGE `show` `img_show` ENUM('1','0')";
$res = mysql_query($sql);*/
...
?>
    <form action="?page=<?=$page?>&zhanr=<?=$zhanr?>" method="post" enctype="multipart/form-data" name="form<?=$e['id']?>">
        <tr bgcolor="<?=$fon?>" valign="top">			
            <td rowspan=5 width="10"><?=$e['id']?></td>
			<td rowspan=5 align="center" width=45>
		...
		<tr bgcolor="<?=$fon?>">
			...
			<? $e["img_show"] == '1' ? $c = 'checked' : $c = '';?>
			<td><input type="checkbox" name="img_show" <?=$c?>></td>
			
		</tr>
        <tr bgcolor="<?=$fon?>">
            <td>Теги</td>
            <td colspan="2"><input type="text" name="tags" value="<?=htmlspecialchars($e["tags"])?>" style="width:300px;"></td>
        </tr>
		<tr bgcolor="<?=$fon?>">
            <td>Ссылка:</td>
