	...
	{	
		...
		if($res && mysql_num_rows($res) > 0)
		{
			...
			while($row = mysql_fetch_array($res))
			{
				...
				$arr[$row['id']] = array(
					'news_date' 	=> format_date_to_str($date[0], $sel_2 = " ", $year = true),
					'news_title'	=> trim(strip_tags($row['news_title'])),
					'news_text_1'	=> trim(strip_tags($row['news_text_1'])),
					'news_text_2'	=> trim($row['news_text_2']),
					'news_img'		=> $html
				);
			}
		}
