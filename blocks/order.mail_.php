...
if(!empty($_POST) && !empty($_SESSION['item']) && isset($_POST['orderPayVar']))
{
	...
	foreach($arrF as $k => $v)
	{
		...
		elseif(in_array($k, $arrNumber))
		{
			...
			else
			{
				switch($k)
				{
					case 'orderDelivDay'		: $oDelivDate['d'] = $_POST[$k]; break;
					case 'orderDelivMonth' 		: $oDelivDate['m'] = $arrMonth[$_POST[$k]];  break;					
					case 'orderClientPhoneCode' : $str = $_POST[$k]; $oClientPhone['c'] = $_POST[$k]; break;
					case 'orderClientPhoneNum'  : $str = $_POST[$k]; $oClientPhone['n'] = $_POST[$k]; $text .= 'Телефон : ('.$oClientPhone['c'].') '.$oClientPhone['n'].'<br>';break;
					default : $str = $_POST[$k]; $text .= $v[0].': '.$str.'<br>'; break;
				}
		...
		else
		{
			switch($k)
			{
				case 'orderDelivVar' 		: $str = $arrDelivVar[$_POST[$k]][0]; 	$text .= $v[0].': '.$str.'<br>'; break;
				case 'orderDelivRange' 		: $str = $arrDelivRange[$_POST[$k]][0]; $text .= $v[0].': '.$str.'<br>'; break;
				case 'orderTestVar' 		: $str = $arrTestVar[$_POST[$k]][0]; 	$text .= $v[0].': '.$str.'<br>'; break;
				case 'orderPayVar' 			: $str = $arrPayVar[$_POST[$k]]; 		$text .= $v[0].': '.$str.'<br>'; break;
				case 'orderClientCard'		: $str = implode(' ', $_POST[$k]);		$text .= $v[0].': '.$str.'<br>'; break;
				case 'orderFullPriceDiscount' : $oClientSum = $str = $_POST[$k];	$text .= $v[0].': '.$str.'<br>'; break;
				case 'orderDelivTime'		: $oDelivDate['t'] = $_POST[$k]; 		$text .= 'Дата доставки : '.@$oDelivDate['d'].' '.@$oDelivDate['m'].' '.@$oDelivDate['t'].'<br>';break;
				default 					: $str = $_POST[$k]; $text .= $v[0].': '.$str.'<br>'; 		break;
			}
	...
	else
	{
		...
		$subject = 'Заказ '.$order_num.' посетителя сайта '.$_SERVER['HTTP_HOST'];
		$message = '<html>
				<head>
				  <title>Заказ '.$order_num.' посетителя сайта</title>
				</head>
				<body>
					'.$text.'
				</body>
