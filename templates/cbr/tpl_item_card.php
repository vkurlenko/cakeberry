	...
	$('.itemInner ul li').hover(function()
		{
			...
			/* сдвиг влево чтобы не выходило за пределы страницы */
			var x = $('#mainDiv').offset().left;
			var lft = $("div.zoom").offset().left + 300 - x;
			
			if(lft > 1190)
			{
				$("div.zoom").offset({left : $(this).offset().left - (lft - 1200 + 10)})
			}
			/* /сдвиг влево чтобы не выходило за пределы страницы */
			
			//alert($("div.zoom").offset().left)
		}, function()
		{
			// спрячем 
			$('.zoom').remove();
